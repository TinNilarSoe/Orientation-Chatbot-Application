    ...
    fetch('/emoji-game')
        ...
        .then(data => {
            if (!data || !Array.isArray(data) || data.length === 0) {
                throw new Error('Invalid emoji game data received');
            }

            console.log('Emoji game data loaded:', data); // Debug log
            emojiGameData = data;
            startEmojiGame();
        })
        .catch(error => {
...
function displayEmojiQuestion() {
    if (!emojiGameData || currentQuestionIndex >= emojiGameData.length) {
        showEmojiCompletion();
        return;
    }
    ...
    // Debug log
    console.log('Displaying emoji question:', question);

    // Check if question has required properties
    if (!question || !question.emoji || !question.question || !question.answer) {
    ...
    emojiDisplay.textContent = question.emoji; // Using textContent to ensure proper emoji rendering
    emojiDisplay.style.fontSize = '4rem';      // Make emoji larger
    emojiDisplay.style.lineHeight = '1.5';     // Proper line height

    // Create question HTML - using DOM methods to ensure proper rendering
    let questionContainer = document.createElement('div');
    questionContainer.className = 'emoji-question';
    questionContainer.setAttribute('data-index', currentQuestionIndex);

    // Append progress indicator
    questionContainer.innerHTML = progressHTML;

    // Append emoji display
    questionContainer.appendChild(emojiDisplay);

    // Create and append question text
    let questionText = document.createElement('div');
    questionText.className = 'emoji-question-text';
    questionText.textContent = question.question;
    questionContainer.appendChild(questionText);

    // Create and append answer form
    let answerForm = document.createElement('div');
    answerForm.className = 'emoji-answer-form';
...
function checkEmojiAnswer(answer) {
    ...
    const isCorrect = answerArray.some(correctAnswer =>
    ...
    if (isCorrect) {
        ...
        feedbackMessage.classList.add('error');
        feedbackMessage.innerHTML = `<i class="fas fa-times-circle"></i> Not quite right. The correct answer was: ${answerArray[0]}`;
    }
